...
namespace JigsawPuzzle
{
    public class Grid : MonoBehaviour
    {
        ...
        public HashSet<Piece> GeneratePieces(PuzzleManager manager)
        {
            ...
            for (int c = 0; c < cols; c++)
            {
                for (int r = 0; r < rows; r++)
                {
                    ...
                    pieceComponent.InjectManager(manager);
        ...
        public bool GetGridSlot(Piece targetPiece, out GridSlot slot)
        {
            int xMax = slots.GetLength(0);
            int yMax = slots.GetLength(1);

            for (int i = 0; i < xMax; i++)
            {
                for (int j = 0; j < yMax; j++)
                {
                    if (slots[i, j].GetPiece().Equals(targetPiece))
                    {
                        slot = slots[i, j];
                        return true;
                    }
